...
npruebasa=$(ls -1 pruebas/[^z]* | wc -l)     # Número de pruebas a ejecutar del autocorrector
valorpruebasa=0.20         # Valor de las pruebas del autocorrector

npruebasc=$(ls -1 pruebas/z* 2>/dev/null | wc -l)               # Número de pruebas del corrector final

#echo "npruebasc= $npruebasc"
#exit 1

valorpruebasc=0.86      # Valor de las pruebas del corrector final
...
g++ -Wall -g -std=c++11 converter.o $PRINCIPAL.cc -o $PRINCIPAL 2> $tempfile
if [ $? -ne 0 ]; then
        cat $tempfile
