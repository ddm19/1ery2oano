...
void CreaIntents(Chatbot &megabot,string linea)
{
	bool separador,repe;
	unsigned i;
	Intent tempIntent;
...
void CreaExample(Chatbot &megabot,string palabra,string tempo)
{
		bool repe=false;
...
void addExample(Chatbot &megabot){
	...
	if(!repe)
		...
		{	
			...
			{
				CreaExample(megabot,palabra,tempo);		
...
void ImportData(Chatbot &megabot)
{
	...
	{
		if(linea[0]=='#')
			CreaIntents(megabot,linea);
		//else
			//CreaExample(megabot,linea);
	}
